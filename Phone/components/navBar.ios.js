...
var NavigationBarRouteMapper = {
    ...
    if (route.name === 'hiredly.me') {
      return (
        <TouchableOpacity style={{alignItems: 'center'}} >
          <Image style={{height: 20, width: 20, margin: 2.5, marginLeft: 10}} source={{uri: icons.hamburger}} />
        </TouchableOpacity> 
      )
    } else {
      return(
        <TouchableOpacity style={{alignItems: 'center'}} onPress={() => navigator.pop()} >
          <Image style={{height: 20, width: 20, margin: 2.5, marginLeft: 10}} source={{uri: icons.back}} />
        </TouchableOpacity> 
      )
    }
  },
  RightButton(route, navigator, index, navState) {
    return (
      <TouchableOpacity style={{alignItems: 'center'}} >
        <Image style={{height: 20, width: 20, margin: 2.5, marginRight: 10}} source={{uri: icons.settings}} />
      </TouchableOpacity> 
    )
  },
  Title(route, navigator, index, navState) {
    return(
      <View style={{flex: 1, alignItems: 'center'}}>
        <Text style={{fontSize: 15, color: '#A92324', margin: 2.5}}>{route.name}</Text>      
      </View>
    )
  }
}
